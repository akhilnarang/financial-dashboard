...
**Net-worth scope is explicit, not implied.** The account picker selects only `Account` rows, while native net worth also contains CAS portfolios and active manual assets/liabilities whose snapshots have no `account_id`. Every preview/generate/sync summary therefore reports deterministic CAS/manual counts and labels plus `cas_investment_scope` (`none`/`excluded`/`partial`/`included`), `cas_investment_coverage` (always `valuation_only` when CAS value is projected), `net_worth_sources_complete` and `net_worth_scope_complete`. `paisa.project_investments=false` explicitly marks CAS excluded. When enabled, CAS is `included` once every portfolio's authoritative value is represented. **Sources complete is not the same as total exact:** `net_worth_scope_complete` additionally requires zero unresolved investment legs, because an unresolved purchase may also sit inside a CAS aggregate (possible double count) and an unresolved redemption is not attributable. Manual items remain prominently outside projection because there is no manual-item selection/mapping contract, so a fully represented CAS never makes `net_worth_scope_complete` true on its own.
...
**CAS is projected as an aggregate valuation only — never as cost-basis lots.** Each CAS portfolio is projected from its authoritative INR `BalanceSnapshot` history and nothing else. The projection does **not** consume `InvestmentLot` rows, emits no commodity lots and no CAS market-price directives. Balances are plain INR with no cost annotation, no acquisition date and no commodity, so no consumer can derive a capital gain or XIRR from them; each posting carries `dashboard_cost_basis_available: false`. `InvestmentLot` itself is untouched — the model, normalization, backfill and dashboard views remain a first-class ingestion fact, and a regression test asserts that populated lot rows produce a byte-identical journal.

**Why cost basis is out of scope here.** Reconciling per-lot cost basis against per-statement CAS aggregates is not determined by this data: a bank row does not name the portfolio it funded, and a CAS value change is purchases minus redemptions **plus market movement**, so the cash-flow component is unrecoverable. Successive attempts to bridge that gap each produced a correctness defect — double counting, assets deleted from net worth, an asset account driven negative, or a later statement rewriting an earlier balance. The conservative boundary is to represent the value exactly and report the ambiguity, not to infer a link. A future cost-basis feature would need a date-effective, monotonic mode timeline (see the preserved design note), which is deliberately out of scope for this PR.

**Statement-effective semantics.** The value source is the one `services/networth.py:current_networth()` selects, so the ledger's CAS component equals the dashboard's by construction rather than by assuming instrument rows sum to the portfolio total (the stated grand total wins; no allocation is invented). The latest snapshot on or before the cutover becomes the opening; each later statement posts only the **delta** since the previous value; a portfolio falling to zero posts the negative of its prior value and clears; an unchanged value posts nothing. Because every posting is dated at its own statement date, a future statement can only affect balances from that date forward — it can never rewrite an earlier one.

**Every bank investment leg is unresolved by policy.** No funding matching, no remapping, no netting against CAS. A purchase keeps its `Assets:Investments:Unallocated` asset (truthfully "invested, portfolio unknown") and is counted in `investment_unresolved_purchases`; a CAS aggregate may also contain it, which is a possible double count an operator must interpret. An unmatched redemption posts to the non-income clearing `Equity:Transfers In` rather than a negative amount on `Assets:Investments:Unallocated` — that account only holds this projection's own unmatched purchases, and a redemption usually disposes of pre-cutover holdings that were never projected, so netting it there would drive an asset account below zero (meaningless accounting, and a fatal Paisa `Negative Balance` diagnosis). The ledger may therefore *exceed* native net worth on the investment component, but never falls below it — visible money rather than missing money — and any unresolved leg makes `net_worth_scope_complete` false.

**Explicit operator mappings outrank every generated policy.** `contra_account()` documents that `paisa.category_mappings` take precedence for every category including investment; the unresolved-redemption reroute honours that, so a configured account is never silently bypassed.

**Portfolio identity is not the portfolio key.** A `portfolio_key` is a PAN, and the generated journal is a file that may be synced or shared, so the account segment is a keyed `HMAC-SHA256(installation_secret, normalized_portfolio_key)` rendered as 80 bits of base32 — never the key itself, and never an unkeyed digest (the PAN space is small enough to enumerate offline). The per-installation secret (`paisa.portfolio_token_secret`) is seeded once by `init_db()`, because projection is strictly read-only and must never create it. With no secret the projection degrades to a single shared, portfolio-less valuation account rather than leaking the key.
...
| `StatementRowDecision` | `statement_row_decisions` | What a person answered about a held statement row: whether it is a separate purchase |
