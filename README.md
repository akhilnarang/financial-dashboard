...
**Card payment traceability.** A bank-side `credit_card_payment` resolves to a specific selected liability when the row carries an explicit `card_id` or an exact `card_mask` matching a selected card account — otherwise it posts to the generic `Liabilities:Credit Card` clearing with `dashboard_card_resolution=unresolved`. **Never** fuzzy/partial matching.
...
| investment-lot (entry-level) | `dashboard_kind`, `dashboard_instrument`, `dashboard_acquired_on`, plus `dashboard_cas_upload_id`/`dashboard_source_ref`/`dashboard_reference` when the source lot states them | `investment_lot`, `INE000A01018`, `2026-01-15` |
...
| `InvestmentLot` | `investment_lots` | Complete, capital-gains-eligible investment lot normalized from an explicit CAS acquisition fact (MF purchase with units+nav+amount+date+isin); never derived from a current value |
