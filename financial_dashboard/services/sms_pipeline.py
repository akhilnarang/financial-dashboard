...
# A cap on the coarse candidate fetch for a completion leg. The predicate is
# already narrow: exact bank, direction, channel, amount, currency, and date.
# More matches than this cannot prove a unique primary. So the fetch stays
# bounded, and hitting the cap makes the completion skip (fail-closed) rather
# than trust a truncated set.
_COMPLETION_COARSE_CAP = 25


class CompletionMatch(NamedTuple):
    """The result of matching a completion leg to its primary row."""

    primary: Transaction | None
    count: int


async def _find_completion_primary(
    session: AsyncSession, txn_data: dict
) -> CompletionMatch:
    """Return the one primary row a completion leg completes, and the count.

    Build the match from the completion leg itself: same bank, direction,
    channel, amount, currency, and date. The primary must already have an
    ...
    reference. Return ``(row, 1)`` on exactly one eligible row. Return
    ``(None, n)`` otherwise. A caller must then fail closed. This is read-only;
    the execution path and the preview path both use it.
    """
    ...
    result = await session.execute(
        ...
        .limit(_COMPLETION_COARSE_CAP + 1)
    )
    coarse = list(result.scalars().all())
    if len(coarse) > _COMPLETION_COARSE_CAP:
        # Too many coarse matches to prove a unique primary. Fail closed.
        return CompletionMatch(None, len(coarse))
    ...
    if len(candidates) != 1:
        return CompletionMatch(None, len(candidates))
    return CompletionMatch(candidates[0], 1)
...
async def _complete_reference(
    ...
    Find the primary with ``_find_completion_primary``. It is fail-closed: on
    zero rows or more than one, this skips and makes no row. On a match, write
    the full reference onto the row. Write the counterparty too when the row has
    none. Then run the reference self-transfer rule, so the event can net
    against its opposite leg. A reparse repeats this safely.
    """
    ...
    # cannot identify its primary row.
    if not reference or not channel:
